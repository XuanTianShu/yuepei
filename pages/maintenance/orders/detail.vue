<template>
	<view class="container" style="height: 100vh;background-color: #fff;">
		...
		<view class="fontButton" style="" >
			<button type="primary" style="" @tap="tofault(datalist.status)">更换电池记录</button>
...
<script>
	import {leaseDeviceDetails} from '@/api/system/maintenance'
	export default{
		data(){
			return{
				datalist:[],
		...
		methods:{
			async getlist(){
				...
				let res = await leaseDeviceDetails(data)
				if(res.code==200){
					
					this.datalist=res.data
					var now= Date.parse(this.datalist.time);
					let endDate = this.datalist.time
					let endTime = endDate
					...
					s = s < 10 ? ('0'+ s) : s
					this.datalist.time=y+'-'+MM+'-'+d+" "+h +':'+ m +':'+ s
