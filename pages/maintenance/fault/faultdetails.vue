			...
			<view class="upload" style="">
				<view class="list" style="">
					...
					<view class="uploadbtn" style="" @click="upload" v-if="imglength<3">
						<image src="https://images.weserv.nl/?url=https://www.yp10000.com/static/img/agent/pic.png" mode="" style=""></image>
					</view>
...
<script>
	import {insertMaintenanceRecord} from '@/api/system/maintenance'
	...
	export default{
		...
		onLoad(option) {
			...
			console.log(this.baseURL,'----',option);
			this.datas=option
		...
		methods:{
			getlist(){
				let data=JSON.parse(this.datas.data)
				this.showlist=data
			...
			async submit(){
				// let img=JSON.stringify(this.feedbackInfo)
				console.log(this.imglist,'=-===');
				// return false
				let s=JSON.stringify(this.imglist)
				let data={
					feedbackId:this.feedbackId,
					feedbackDescribe:this.feedbackInfo,
					devicePicture:s,
				}
				console.log(data,'---');
				// return false;
				let res=await insertMaintenanceRecord(data)
				if(res.code==200){
					uni.showToast({
						title: res.msg,
						duration: 2000
					...
					data.status='1'
					data.devicePicture=JSON.stringify(this.imglist)
					data.feedbackInfo=this.feedbackInfo
					data.deviceAddress=this.showlist.deviceAddress
					console.log(data);
					// return false 
					let list=JSON.stringify(data)
					uni.reLaunch({
						url:'faultfinish?list='+list
					})
				}
			},
			deleteimg(item){
