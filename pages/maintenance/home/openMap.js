...
function openMapByDefault(latitude, longitude, name, address) {
	uni.openLocation({
		latitude: latitude,
		longitude: longitude,
		name: name,
...
function openMapByAndroid(latitude, longitude, name, address) {
	let url = ''; // 回调地址
	let identity = ''; // 程序名称
	if(plus.runtime.isApplicationExist({pname: 'com.baidu.BaiduMap'})) { // baidumap
		...
		openMapByDefault(latitude, longitude, name, address)
	}
}
...
export default {
	/* 打开地图 */
	openMap(latitude, longitude, name, address, coord_type='gcj02') {
		let arr = getCoordByType(longitude, latitude, coord_type)
		// #ifdef APP-PLUS
		switch(uni.getSystemInfoSync().platform){
			case 'android':
				console.log('运行Android上')
				openMapByAndroid(arr[1], arr[0], name, address)
				break;
			case 'ios':
				console.log('运行iOS上')
				...
				break;
			default:
				openMapByDefault(arr[1], arr[0], name, address)
