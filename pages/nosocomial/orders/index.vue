		...
		methods:{
			...
			ToDetail(status,orderNumber){
				if(status==0){
					// 租赁中
					uni.navigateTo({
						url:"/pages/agent/commodity/Rental?orderNumber="+orderNumber
					})
				}else if(status==1){
					// 待支付
					uni.navigateTo({
						url:"/pages/agent/commodity/bepaid?orderNumber="+orderNumber
					})
				}else if(status==2){
					// 支付完成
					console.log('===');
					uni.navigateTo({
						url:"/pages/agent/commodity/Completedpackage?orderNumber="+orderNumber
					})
				}else if(status==4){
					console.log('jajaj');
					uni.navigateTo({
						url:"/pages/agent/commodity/refund?orderNumber="+orderNumber
					})
