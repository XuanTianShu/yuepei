	...
	<view class="containet" style="">
		<view class="head" style="">
			<view class="search-bar">
				...
				<input type="text" value="" v-model="Number"  placeholder="谓输入阵品名称、订单编号" class="search-text" maxlength="30" focus placeholder-style="color:#d1d1d1;"/>
				<button class="search-btn" style="" @click="search">搜索</button>
				</view>
			</view>
		...
		<view class="list" style="">
			<view class="list-title" style="">
				<view class="" @tap="num=5" :class="num==5?'green':'red'" @click="getstatus()">全部</view>
				<view class="" @tap="num=0" :class="num==0?'green':'red'" @click="getstatus(0)">租借中</view>
				<view class="" @tap="num=1" :class="num==1?'green':'red'" @click="getstatus(1)">待支付</view>
				<!-- <view class="" @tap="num=3" :class="num==3?'green':'red'" @click="getstatus(3)">审核中</view> -->
				<view class="" @tap="num=2" :class="num==2?'green':'red'" @click="getstatus(2)">已完成</view>
				<view class="" @tap="num=4" :class="num==4?'green':'red'" @click="getstatus(4)">已退款</view>
			</view>
			...
			<view class="list-content" style="" v-for="item in list">
				...
				<view class="list-item items"  style="">
					...
					<view class="" v-else-if="item.status==2" style="color: #f09a74;">
						支付完成
					</view>
				...
				<view class="list-item items" style="">
					<view class="">
						支付金额
						<text style="color: #feb196;" v-if="item.price==null">0.00</text>
						<text style="color: #feb196;" v-else>{{item.price}}</text>
					</view>
...
<script>
	import {selectLeaseOrder,selectDepartment,selectDeviceList} from '@/api/system/agent'
	...
	export default{
		...
		methods:{
			async search(){
				this.nameOrNumber=this.Number
				let deviceDep
				if(this.deviceDepartment==undefined){
					deviceDep=''
				}else{
					deviceDep=this.deviceDepartment
				}	
				let name
				...
				if(res.code==200){
					...
					for(let i in res.data){
						...
						let endDate = res.data[i].leaseTime
							  ...
							  let restore = new Date(res.data[i].restoreTime)
							    let yy = restore.getFullYear() // 年
							    let MMM = restore.getMonth() + 1 // ⽉
							    MMM = MMM < 10 ? ('0' + MMM) : MMM
							    let dd = restore.getDate() // ⽇
							    dd = dd < 10 ? ('0' + dd) : dd
							    let hh = restore.getHours() // 时
							    hh = hh < 10 ? ('0' + hh) : hh
							    let mm = restore.getMinutes()// 分
							    mm = mm < 10 ? ('0' + mm) : mm
							    let ss = restore.getSeconds()// 秒
							    ss = ss < 10 ? ('0' + ss) : ss
							  res.data[i].restoreTime=yy+'-'+MMM+'-'+dd+" "+hh + ':' + mm + ':' + ss
							  this.statuList[i].restoreTime=res.data[i].restoreTime
					}
				}
			},
			...
			ToDetail(status,orderNumber){
				...
				if(status==0){
					...
					console.log('status');
					uni.navigateTo({
						url:"refund?orderNumber="+orderNumber
					})
				}else{
					// 支付完成
					uni.navigateTo({
			...
			async getLease(){
				...
				let data={
					userId:userid,
					nameOrNumber:this.nameOrNumber,
					deviceTypeNamue:this.deviceTypeNamue,
				...
				if(res.code==200){
					...
					for(let i in res.data){
						...
						let endDate = res.data[i].leaseTime
							  ...
							  // var nows=Date.parse(res.data[i].leaseTime);
							  // let endDate = res.data[i].leaseTime
							        // let endTime = endDate
							        let restore = new Date(res.data[i].restoreTime)
							        let yy = restore.getFullYear() // 年
				...
				let regs=await selectDeviceList(userId)
				if(regs.code==200){
					console.log('keshi');
					for(let i in regs.data){
						let res={text:'',value:''}
						res.text=regs.data[i].deviceTypeName
						res.value=regs.data[i].deviceTypeId
