<template>
	<view class="uni-select-cy" :style="{'z-index':zindex}">
		<view class="uni-select-cy-select" :class="{ active: active }" @click.stop="handleSelect">
			...
			<!-- 为空时的显示文案 -->
			<view style="margin-right: 40%;" v-if="realValue.length == 0&&showplaceholder">{{ placeholder }}</view>
			<!-- 禁用图标 -->
		...
		<scroll-view class="uni-select-cy-options" :scroll-y="true" v-show="active" @scrolltolower="scrolltolower">
			<template>
				...
				<view class="" v-if="options==''" style="margin-left: 30rpx;">
					暂无数据
				</view>
				<view class="uni-select-cy-item" v-else :class="{ active: realValue.includes(item[svalue]) }"
